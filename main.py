...
def download(symbol, date, days=365):
               ...
               #Rename columns
               df_source = df_source.rename(columns={"Open": "open", "High": "high", "Low": "low", "Close": "Close", "Adj Close": "adj_close", "Volume": "volume"})
...
# %%
def ElliottWaveFindPattern(df_source, measure, granularity, dateStart, dateEnd, today, extremes=True):
    ...
    for k, v in waves_for_len.items():
        result = findBestFitWave(df_samples, measure, v)  # Note: v here is the value
        print("best fit")
        print(result)
        draw_wave(df, df_samples, result)


ElliottWaveFindPattern(df_source, "Close", 7, "2019-03-01", today, today, extremes=False)
